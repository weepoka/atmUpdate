...
const Visaquery = () => {
      ...
      <div key={i} className="p-4 w-[300px] shadow-xl rounded-md border  ">
        ...
        <div className=" p-2 my-2 rounded-md bg-[#343e4e] shadow-lg ">
          ...
          <p className="text-pink-200">
            <span className="text-cyan-700 font-medium mr-2">Reading:</span>{" "}
            {info?.ieltRead}
          </p>
          <p className="text-pink-200">
            <span className="text-cyan-700 font-medium mr-2">Writing:</span>{" "}
            {info?.ieltWrite}
          </p>
          <p className="text-pink-200">
            <span className="text-cyan-700 font-medium mr-2">Speaking:</span>{" "}
            {info?.ieltSpeak}
          </p>
          <p className="text-pink-200">
            <span className="text-cyan-700 font-medium mr-2">Listening:</span>
            {info?.ieltListen}
          </p>
          <p className="text-pink-200">
            <span className="text-cyan-700 font-medium mr-2">Over View:</span>
            {info?.ieltOver}
          </p>
        </div>
        ...
        <div className="flex justify-between">
          <p
            onClick={() => {
              setData({ ...data, id: info._id });
              setAppp(!appp);
            }}
            className="text-end text-green-500 font-semibold text-base cursor-pointer"
          >
            Approve
          </p>
  ...
  return (
    <div className=" m-7 lg:w-full w-[60%] text-white">
      <p className="text-center font-medium text-cyan-700 mt-8">
        {contt?.message ? contt?.message : "NO VISA QUERY"}
      </p>
      {contt?.message && (
        <>
          <div className=" flex flex-wrap gap-10">{ContCard}</div>
          {appp && (
            <div className=" mb-7 mt-8">
              <div className="">
                ...
                <div>
                  ...
                  <button
                    onClick={appfun}
                    className="block text-green-500 text-center w-[20%] mt-2 rounded-md py-[10px] border-deep-orange-500 font-semibold border  text-base cursor-pointer"
                  >
                    SEND
                  </button>
                </div>
      ...
      <div className=" flex flex-wrap gap-10 ">{AppCard}</div>
    </div>
  );
};
