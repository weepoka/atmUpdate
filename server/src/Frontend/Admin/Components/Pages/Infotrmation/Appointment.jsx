    ...
    const ContCard = cont && cont?.map((info, i) => (
        <div key={i} className="p-4 w-[300px] shadow-xl rounded-md border  ">
            ...
            <div className='my-2 '>
                ...
                <p className='text-purple-200 '>Visiting approve: {info?.approve ? "Confirm" : " NO"}</p>
            ...
            <p className='inline-block mr-2'>Visiting Date:</p><p className='inline-block'> {info?.visitingDate?.split("T")[0]}</p>
            <p className='inline-block mr-2'>Confirm Date:</p><p className='inline-block'> {info?.confirmDate?.split("T")[0]}</p>
            <div className=' flex justify-between items-center mt-4'>
                {up &&
                    <p onClick={() => replayF(info?._id)} className='cursor-pointer font-medium text-green-600 rounded-md shadow-lg bg-[#2a3544]'>Update Now</p>}
    ...
    return (
        <div className="m-7 lg:w-full w-[60%] text-white">
            ...
            <div className='flex gap-x-4 p-4'>
                <button onClick={() => (contactList("appointment-today"), setUp(true), setToday(false), setShow(false))} className="bg-blue-600 rounded-md text-center  px-10 py-2">
                    Today
                </button>
                <button onClick={() => (contactList("appointment-next"), setUp(false), setShow(false))} className="bg-blue-600 rounded-md text-center  px-10 py-2">
                    Next
                </button>
                <button onClick={() => (contactList("appointment-ex"), setUp(true), setToday(true), setShow(false))} className="bg-blue-600 rounded-md text-center  px-10 py-2">
                    Waiting For Approve
                </button>
                <button onClick={() => (contactList("appointment-del"), setUp(false), setShow(false))} className="bg-blue-600 rounded-md text-center  px-10 py-2">
                    Expired Appointment
                </button>
            </div>
            {card &&
            ...
            }
            {
                        ...
                        {today &&
                            <div className=" mb-7 px-5">
                                <div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-20">
                                    {" "}
                                    <div>
                                        <label className="mb-2 md:text-lg  text-gray-400">Confirm Date</label>
                                    </div>{" "}
                                    <div>
                                        <input
                                            type="date"
               ...
               rounded w-full max-w-xs lg:max-w-lg xl:max-w-xl 2xl:min-w-full"
                        ...
                        <div className=" mb-7 px-5">
                            <div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-20">
                                {" "}
                                <div>
                                    <label className="mb-2 md:text-lg  text-gray-400">Has Visited</label>
                                </div>{" "}
                                <div>
                                    <select
               ...
               rounded w-full max-w-xs lg:max-w-lg xl:max-w-xl 2xl:min-w-full"
                    ...
                    <div className="text-center pt-10">
                        <button onClick={sendReplay} className="bg-blue-600 rounded-md text-center  px-10 py-2">
                            Send
                        </button>
                    </div>
                </div>}
